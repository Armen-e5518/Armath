    ...
    $.ajax({
        ...
        success: function (res) {
            if (res) {
                ...
                res.all_labs.forEach(function (val, i_all) {
                    var schools = [];
                    val.labs.forEach(function (labs, i_school) {
                        console.log(labs.uuid)
                        schools_array.push(labs.uuid)
                        schools.push(labs.uuid)
                        if (i_school == val.labs.length - 1) {
                            $('#id_regions').append(
